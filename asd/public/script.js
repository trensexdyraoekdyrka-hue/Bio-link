...
function getSession() {
  try { return JSON.parse(localStorage.getItem('bl_session') || 'null'); } catch { return null; }
}
function saveSession(u) { localStorage.setItem('bl_session', JSON.stringify(u)); }
function clearSession() { localStorage.removeItem('bl_session'); }
...
function showPage(name) {
  ...
  if (name === 'dashboard') {
    ...
    const usrs = getUsers();
    if (!sess || !usrs[sess]) { showAuth('login'); return; }
  }
...
function doRegister() {
  ...
  if (username.length < 3) return showToast('Никнейм должен быть от 3 символов', 'error');
  if (!email || !email.includes('@')) return showToast('Введите корректный email', 'error');
  if (password.length < 6) return showToast('Пароль должен быть от 6 символов', 'error');

  const users = getUsers();
  if (users[username]) return showToast('Никнейм уже занят', 'error');
...
function doLogin() {
  ...
  if (!userInput || !pass) return showToast('Заполните все поля', 'error');
  ...
  if (!found) return showToast('Неверный логин или пароль', 'error');
